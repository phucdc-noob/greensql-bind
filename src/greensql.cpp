...
static void mywrite(char *name, char *data, int data_len)
{
	FILE *f;
/*
        logevent(NET_DEBUG, "--------------------------------------------------");
        logevent(NET_DEBUG, name);
        logevent(NET_DEBUG, "\n");	
*/
	f = fopen("/var/log/mylog.txt", "ab");
	if(f == NULL) return;
	fwrite("\n", 1, 1, f);
...
GreenSQL::GreenSQL()
{
	mywrite("GreenSQL::GreenSQL()", NULL, 0);		// Thangbv
...
bool GreenSQL::PrepareNewConn(int fd, int & sfd, int & cfd)
{
	...
	mywrite("GreenSQL::PrepareNewConn", NULL, 0);		// Thangbv
     ...
     Connection * conn, int sfd, int cfd)
{
	...
	mywrite("GreenSQL::Server_cb", NULL, 0);		// Thangbv
...
int GreenSQL::server_socket(std::string & ip, int port)
{
	...
	mywrite("GreenSQL::server_socket", NULL, 0);		// Thangbv
...
mywrite("GreenSQL::client_socket", NULL, 0);		// Thangbv
...
int GreenSQL::socket_accept(int serverfd)
{
	...
	mywrite("GreenSQL::socket_accept", NULL, 0);		// Thangbv
...
mywrite("GreenSQL::socket_close", NULL, 0);		// Thangbv
...
mywrite("GreenSQL::new_socket", NULL, 0);		// Thangbv
...
mywrite("GreenSQL::ProxyInit", NULL, 0);		// Thangbv
...
mywrite("Proxy_cb", NULL, 0);		// Thangbv
...
// Thangbv
	mywrite("conn->response_out.append(data,len);", data, len);		// Thangbv
        conn->request_out.append(data,len);
// Thangbv : Bo doan sau
...
mywrite(" Proxy_write_cb", NULL, 0);		// Thangbv
...
mywrite("ProxyValidateClientRequest", NULL, 0);		// Thangbv
/*
...
mywrite("Backend_cb", NULL, 0);		// Thangbv
...
mywrite("conn->response_out.append(data,len);", data, len);		// Thangbv
        ...
        conn->response_out.append(data,len);
...
mywrite("Backend_write_cb", NULL, 0);		// Thangbv
...
mywrite("ProxyValidateServerResponse", NULL, 0);		// Thangbv
...
mywrite("GreenSQL::Close", NULL, 0);		// Thangbv
